...
const main = async () => {
  scriptStatus('Start');
  const data = await fetchByPinWeekWise(constants.pincodes, constants.rateLimitPrecaution);
  const filteredData = data
    .filter((session) => dataFilter(session, constants.filters))
    .sort((session1, session2) => dataSort(session1, session2, constants.filters));
  logger('data', data.map(loggerFilter));
  logger('filteredData', filteredData.map(loggerFilter));

  if (filteredData.length > 0) {
    await openCowin(filteredData, constants.mobile, constants.alarm);
  }
